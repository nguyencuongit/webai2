<section {!! $shortcode->htmlAttributes() !!} class="tp-subscribe-area pt-70 pb-65 theme-bg p-relative z-index-1" @if ($shortcode->background_image) style="background-image: url({{ RvMedia::getImageUrl($shortcode->background_image) }}) !important; background-repeat: no-repeat !important; background-size: cover !important;" @endif>
    <div class="tp-subscribe-shape d-none d-sm-block">
        @if ($shortcode->use_shape_images)
            @for($i = 1; $i <= 4; $i++)
                {{ RvMedia::image(Arr::get($shortcode, "shape_$i") ?: Theme::asset()->url("images/newsletter/shape-$i.png"), $shortcode->title, attributes: ['class' => "tp-subscribe-shape-$i"]) }}
            @endfor
        @endif

        @if ($shortcode->use_air_paper_icon_with_animation)
            <div class="tp-subscribe-plane">
                <img class="tp-subscribe-plane-shape" src="{{ Theme::asset()->url('images/newsletter/plane.png') }}" alt="{{ $shortcode->title }}">
            ...
            <div class="col-xl-7 col-lg-7">
                <div class="tp-subscribe-content">
                    @if($shortcode->subtitle)
                        <span>{!! BaseHelper::clean($shortcode->subtitle) !!}</span>
                    @endif
                    @if($shortcode->title)
                        <h3 class="tp-subscribe-title">{!! BaseHelper::clean($shortcode->title) !!}</h3>
                    @endif
                </div>
            </div>
