<section class="tp-subscribe-area pt-70 pb-65 theme-bg p-relative z-index-1" @if ($config['background_image']) style="background-image: url({{ RvMedia::getImageUrl($config['background_image']) }}) !important; background-repeat: no-repeat !important; background-size: cover !important;" @endif>
    <div class="tp-subscribe-shape d-none d-sm-block">
        @if ($config['use_shape_images'])
            @for($i = 1; $i <= 4; $i++)
        ...
        @if ($config['use_air_paper_icon_with_animation'])
        ...
        <div class="row align-items-center">
            <div class="col-xl-7 col-lg-7">
                <div class="tp-subscribe-content">
                    ...
                    @endif
                    @if($config['title'])
                        <h3 class="tp-subscribe-title" style=" text-shadow:0 0 2px rgba(0,0,0,.65), 0 0 6px rgba(0,0,0,.35), 0 2px 10px rgba(0,0,0,.25);">{!! BaseHelper::clean($config['title']) !!}</h3>
                    @endif
                </div>
            </div>
