...
use Botble\Payment\Http\Controllers\PaymentTransactionReportController;
use Illuminate\Support\Facades\Route;

AdminHelper::registerRoutes(function (): void {
    Route::get('payments/reports/transactions', [PaymentTransactionReportController::class, 'index'])
        ->name('payments.reports.transactions')
        ->middleware('preventDemo')
        ->permission('payments.reports.transactions');

    Route::get('payments/reports/transactions/{paymentId}', [PaymentTransactionReportController::class, 'show'])
        ->whereNumber('paymentId')
        ->name('payments.reports.transactions.show')
        ->permission('payments.reports.transactions');

    Route::post('payments/reports/transactions/{paymentId}/confirm', [PaymentTransactionReportController::class, 'confirm'])
        ->whereNumber('paymentId')
        ->name('payments.reports.transactions.confirm')
        ->permission('payments.reports.transactions');
    ...
    Route::group(['prefix' => 'payments/logs', 'as' => 'payments.logs.', 'permission' => 'payments.logs'], function (): void {
        ...
        Route::post('backfill', [PaymentLogController::class, 'backfill'])->name('backfill');
